...
public class PariRestController {
	...
	// get match by Id
				...
				@PathVariable("pariId") Long pariId) 
		{
			...
			utilisateurService.getOne(utilisateurId).orElseThrow(
					() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Utilisateur "
							+ "not found with id: " + utilisateurId));

			Pari pari = pariService.getOnePariByUtilisateur(pariId, utilisateurId).orElseThrow(
					() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Pari not found with id: " 
			+ pariId));
				...
				@RequestBody PariDto pariDto) 
		{
			...
			pari = pariService.savePariByUtilisateur(utilisateurId, pari)
					.orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Utilisateur non trouvée :" + utilisateurId));
		...
		public ResponseEntity<PariDto> editOnePariByUtilisateur(@PathVariable("utilisateurId") Long utilisateurId,
			...
			utilisateurService.getOne(utilisateurId).orElseThrow(
					() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Utilisateur" + " not found with id: " + utilisateurId));

			Pari pari = pariService.editOnePari(pariId, utilisateurId, pariDto).orElseThrow(
